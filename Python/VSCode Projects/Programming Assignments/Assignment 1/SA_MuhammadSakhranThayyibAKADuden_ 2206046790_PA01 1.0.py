...
if petalNumber==funnysexnumber:
    turtle.penup()
    turtle.goto(100,0)
    turtle.pendown()
    turtle.color(random.random(), random.random(), random.random())
    turtle.write("Nice", align="center", font=("Comic Sans MS", 14, "normal"))

if rows==funnysexnumber:
    turtle.penup()
    turtle.goto(100,0)
    turtle.pendown()
    turtle.color(random.random(), random.random(), random.random())
...
for i in range(rows):
    for i in range(rows):
            ...
            t.end_fill()
    t.penup()
    t.setpos(-(sizeNumberThing/2), t.ycor()+squareSize) #collabed with Muhammad Oka, the OG
    t.pendown()
